...
public:
	...
	// Constructors
	ctDynArray() : mem_capacity(0), num_elements(0), data(NULL)
	{
		Alloc(DYN_ARRAY_BLOCK_SIZE);
	}

	ctDynArray(unsigned int capacity) : mem_capacity(0), num_elements(0), data(NULL)
	{
		Alloc(capacity);
	}
	...
	const ctDynArray<VALUE>& operator+= (const ctDynArray<VALUE>& array)
	{
	...
	bool Insert(const ctDynArray<VALUE>& array, unsigned int position)
	{
		if (position > num_elements)
			return false;
