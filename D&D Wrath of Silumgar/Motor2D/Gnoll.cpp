...
Gnoll::Gnoll(int x, int y, EntityType type) : Entity(x, y, type) {
...
void Gnoll::SetPlayerAnimationsSpeed(float dt)
{
	idle.speed = idle_vel * dt;
	run_forward.speed = run_forward_vel * dt;
}

void Gnoll::SetEntitiesSpeed(float dt)
{
	idle_vel = idle.speed;
...
void Gnoll::LoadAnimation(pugi::xml_node animation_node, ctAnimation* animation)
{
	bool ret = true;
