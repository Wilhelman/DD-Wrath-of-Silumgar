...
UICombatMenu::UICombatMenu(Entity* entity, int x, int y, UI_Type type, ctModule* callback, UIElement* parent) : UIElement(x, y, type, parent)
{
	...
	if (entity->abilities.size() != 0) {
		...
		abilities_label = App->gui->AddUITextBox(main_label2_pos.x,main_label2_pos.y, font_size, 500, "Abilities", { 255,0,0,255 }, background);
	}
	items_label = App->gui->AddUITextBox(main_label3_pos.x, main_label3_pos.y, font_size, 500, "Items", { 255,0,0,255 }, background);
	attack_label->current_state = STATE_FOCUSED;
	explanation_label = App->gui->AddUITextBox(2, 1, 15, 224, ATTACKEXPLANATION, { 255,255,255,255 }, explanation_background, Second_Font);
	arrow = App->gui->AddUIImage(-20, 0, { 1333, 272, 7, 14 }, callback, attack_label);
	...
	if (current_vector == main_labels) {
		...
		if (names.size() > current_vector.size() && it_vector == current_vector.end() && names_iterator < names.size() - 1) {
			...
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator - 1).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x,label1_pos.y, font_size, 500, names.at(names_iterator - 1), { 255,255,255,255 },background));
			}
			else {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(names_iterator - 1), { 255,0,0,255 }, background));
			}
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label2_pos.x, label2_pos.y, font_size, 500, names.at(names_iterator), { 255,255,255,255 }, background));
			}
			else {
				current_vector.push_back(App->gui->AddUITextBox(label2_pos.x, label2_pos.y, font_size, 500, names.at(names_iterator), { 255,0,0,255 }, background));
			}
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator + 1).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label3_pos.x,label3_pos.y, font_size, 500, names.at(names_iterator), { 255,255,255,255 }, background));
			}
			else {
				current_vector.push_back(App->gui->AddUITextBox(label3_pos.x, label3_pos.y, font_size, 500, names.at(names_iterator), { 255,0,0,255 }, background));
			}
			names_iterator++;
	...
	if (current_vector == main_labels) {
		...
		if (names.size() > current_vector.size() && it_vector == current_vector.end() && names_iterator > 2) {
			...
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator - 3).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(names_iterator - 3), { 255,255,255,255 }, background));
			}
			else {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(names_iterator - 3), { 255,0,0,255 }, background));
			}
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator - 2).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(names_iterator - 2), { 255,255,255,255 }, background));
			}
			else {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x,label1_pos.y, font_size, 500, names.at(names_iterator - 2), { 255,0,0,255 }, background));
			}
			if (entity->GetCurrentManaPoints() >= entity->abilities.at(names_iterator - 1).mana_points_effect_to_himself) {
				current_vector.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(names_iterator - 1), { 255,255,255,255 }, background));
			}
...
void UICombatMenu::LoadAbilities() {
	...
	if (names.size() > 3) {
		if (entity->GetCurrentManaPoints() >= entity->abilities.at(0).mana_points_effect_to_himself) {
			abilities.push_back(App->gui->AddUITextBox(label1_pos.x, label1_pos.y, font_size, 500, names.at(0), { 255,255,255,255 }, background));
		}
		else {
			abilities.push_back(App->gui->AddUITextBox(label1_pos.x,label1_pos.y, font_size, 500, names.at(0), { 255,0,0,255 }, background));
		}
		if (entity->GetCurrentManaPoints() >= entity->abilities.at(1).mana_points_effect_to_himself) {
			abilities.push_back(App->gui->AddUITextBox(label2_pos.x, label2_pos.y, font_size, 500, names.at(1), { 255,255,255,255 }, background));
		}
		else {
			abilities.push_back(App->gui->AddUITextBox(label2_pos.x,label2_pos.y, font_size, 500, names.at(1), { 255,0,0,255 }, background));
		}
		if (entity->GetCurrentManaPoints() >= entity->abilities.at(2).mana_points_effect_to_himself) {
			abilities.push_back(App->gui->AddUITextBox(label3_pos.x, label3_pos.y, font_size, 500, names.at(2), { 255,255,255,255 }, background));
		}
		else {
			abilities.push_back(App->gui->AddUITextBox( label3_pos.x, label3_pos.y, font_size, 500, names.at(2), { 255,0,0,255 }, background));
		}
		names_iterator = 2;
	}
