#ifndef __ctSString_H__
#define __ctSString_H__
...
public:
	...
	ctSString(const ctSString& string)
	{
		Alloc(string.size);
		strcpy_s(str, size, string.str);
	}

	ctSString(const char *format, ...)
	{
		size = 0;
	...
	const ctSString& create(const char *format, ...)
	{
		size = 0;
	...
	// Operators
	bool operator== (const ctSString& string) const
	{
	...
	const ctSString& operator= (const ctSString& string)
	{
	...
	const ctSString& operator= (const char* string)
	{
		ctSString t(string);
		(*this) = t;
		return *this;
	...
	*/
	uint SubString(unsigned int start, unsigned int end, ctSString& buffer) const
	{
		if (str != NULL)
		{
