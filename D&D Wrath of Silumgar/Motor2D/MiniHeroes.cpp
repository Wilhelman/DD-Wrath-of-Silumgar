...
MiniHeroes::MiniHeroes(int x, int y, EntityType type) : Entity(x, y, type) {

	bool ret = true;

	pugi::xml_document	config_file;
	pugi::xml_node* node = &App->LoadEntities(config_file);
	node = &node->child("heroes").child("miniheroes");
	texture = App->tex->Load(App->entities->miniheroes_spritesheet_name.data());
...
void MiniHeroes::SetPlayerAnimationsSpeed(float dt)
{
	idle.speed = idle_vel * dt;
	run_forward.speed = run_forward_vel * dt;
}

void MiniHeroes::SetEntitiesSpeed(float dt)
{
	idle_vel = idle.speed;
...
void MiniHeroes::LoadAnimation(pugi::xml_node animation_node, ctAnimation* animation)
{
	bool ret = true;
